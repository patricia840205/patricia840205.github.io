...
//	echo "留言編號: " . $row["留言編號"] . " - 姓名: " . $row["姓名"] . " - 留言: " . $row["留言"]. " - 日期時間: " . $row["日期時間"] . " - 貼文編號: " . $row["貼文編號"] . " - topic_id: " . $row["topic_id"] . "<br>";
 // }
  $sql = "SELECT * FROM produce WHERE 貼文編號 = '{$_GET['saledetailed']}'";
  $result=mysqli_query($conn, $sql);
  $thing=mysqli_fetch_array($result);
  $sql = "SELECT * FROM message WHERE topicid = '{$_GET['saledetailed']}'";
...
<div id="maintext" style="border-bottom: dashed #000; ">
      <?php 
	   ...
	   <h3 style='background-color:#8585ad; text-align:left;'><strong>&nbsp;※".$thing['goodname']."</h2></strong>
	   <p style='background-color:lightGrey; text-align:left;'>
	   <br>分類:&nbsp;&nbsp;".$thing['type']."
	   <br>學校:&nbsp;&nbsp;".$thing['school']."
       <br>價錢:$ &nbsp;&nbsp;".$thing['price']."
	   <br>圖片網址:&nbsp;&nbsp;".$thing['url']."
	   <br>介紹:&nbsp;&nbsp;".$thing['introduce']."</p>
  ...
  <div id="navigation"><a href="message.php?saledetailed=<?php echo $_GET['saledetailed']; ?>">我要留言</a></div>
...
if ($numRows>0) {
  ...
  //var_dump($thisr);
	...
	if (isset ($_SESSION['user_id']) && @($_SESSION['yourname'])==$name) {
		 echo "<a href='messageedit.php?messageedit={$thisr['留言編號']}' style='right:350px; position: absolute; '>編輯&nbsp;</a>";
	}
	...
	if (isset($_SESSION['user_id']) && @($_SESSION['yourname']==$name) || @($row['姓名'] == $_SESSION['yourname'])) {
		echo "<a href='msgdelete.php?msgdelete={$thisr['留言編號']}' style='right:400px; position: absolute;;' type='hidden;'>刪除</a>";
	}
