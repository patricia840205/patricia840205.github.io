...
if ($errMsg ==''){
  ...
  $stmt = mysqli_prepare($conn,                 //準備查詢
               "INSERT INTO $myTable (`姓名`, `留言`, `日期時間`, `topic_id`)
                VALUES (?, ?, ?, ?)");
  $now = date("Y-m-d H:i:s");
  // 繫結參數
  
  mysqli_stmt_bind_param($stmt, 'sssi', $name, $message, $now, $_POST['topic_id']);
...
?>
    <p><a href="detailed.php?detailed=<?php echo $_POST['topic_id']; ?>" style="float:right;">回留言板</a></p>
