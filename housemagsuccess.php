...
//檢查是否已輸入姓名和留言 
 if ( !empty($_SESSION['user_id']) && !empty($_POST['housename']) && !empty($_POST['price']) && !empty($_POST['type']) && !empty($_POST['school']) && !empty($_POST['address']) && !empty($_POST['introduce'])) {
  //將姓名放入 $name 變數
  //$name=myStripslashes($_POST['name']);
  $name = $_SESSION['yourname'];
  $goodname = $_POST['housename'];
  //將留言放入 $message 變數
...
if ($errMsg ==''){
  ...
  $stmt = mysqli_prepare($conn,                 //準備查詢
               "INSERT $myTable (`姓名`,`housename`, `price`, `type`, `school`,  `address`, `introduce`, `date`)
                VALUES (?, ?, ?, ?, ?, ?,?,?)");
  ...
  mysqli_stmt_bind_param($stmt, 'ssisssss', 
                         $name, $housename, $price, $type, $school, $address, $introduce, $date);
